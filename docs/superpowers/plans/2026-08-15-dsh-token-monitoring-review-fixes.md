...
Replace “`renderer/` 一行不改” with “不新增 DSH 平台入口或独立面板;允许修改现有 cost-line 数据合并路径”。 Update the self-review statement that claims all renderer files are untouched. Do not rewrite historical task bodies beyond adding the supersession notice.
