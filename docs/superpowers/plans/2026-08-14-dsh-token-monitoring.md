...
- 已知偏差(有意为之,已向用户说明):DSH writer 用 `appendFile` 每行重开文件而非缓存句柄——轮转语义等价、无句柄失效状态;`usageDailyCost` 为计划新增的 store 键(spec 4.3/4.4 的费用落地载体),经 `sanitizeSettings` 白名单外放行,渲染层可读。
