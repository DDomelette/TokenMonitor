...
test('default compact layout keeps stat cards as square tiles above full-width charts', () => {
  ...
  assert.deepEqual(
    ['model-bar', 'token-line', 'cost-line'].map((id) => ({ x: byId[id].x, w: byId[id].w })),
    [
      { x: 0, w: 12 },
      { x: 0, w: 12 },
      { x: 0, w: 12 }
...
test('migration preserves compact component order', () => {
  ...
  assert.deepEqual(
    ...
    ['cost-line', 'balance-card', 'today-cost-card', 'cache-rate-card', 'model-bar', 'token-line']
  );
  assert.deepEqual(
...
test('validation removes unknown and duplicate ids but restores missing records', () => {
  ...
  assert.deepEqual(
    ...
    ['balance-card', 'cache-rate-card', 'cost-line', 'model-bar', 'today-cost-card', 'token-line']
  );
  assert.equal(new Set(result.items.map((item) => item.id)).size, 6);
});
